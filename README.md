...
项目已完成 Codex 与智谱的剩余额度、重置时间显示，支持通过 BOOT 按键或 GPIO13
高电平外接按键在两个服务商页面间切换。商汤日日新、OpenRouter 免费额度等服务商
列入后续版本。

> 当前状态：v0.2 已完成。PC Agent 的 81 项单元测试与 Codex／智谱真实 HTTP 冒烟
> 均通过；固件已完成 Arduino IDE 编译、烧录，以及双服务商页面、BOOT 切换、断网、
> 过期、错误恢复、中文布局和重置倒计时实机验收。BOOT 切页延迟的软件修复已完成，
> 有效按键会立即唤醒 UI，切页后再立即唤醒网络任务；固件已重新编译、烧录，连续
> 12 次短按均在按键确认的同一毫秒切页，Codex／智谱往返正确，长按也只触发一次，
> 该缺陷已完成实机回归并关闭。GPIO13 外接按键的软件实现和固件语法检查已完成，
> 新固件已编译、烧录，GPIO13 与原 BOOT 的单击双向切页、长按单次触发实机验收
> 全部通过。验证记录见 `ROADMAP.md`。

## 项目目标

- 在 OLED 上以低认知成本显示 AI 服务商的额度状态。
- 将不同服务商的数据统一为稳定的额度快照，避免显示层绑定某一家服务商的接口。
- 使用 BOOT 按键或 GPIO13 高电平外接按键切换服务商页面；两颗按键执行相同操作。
...
- 目标数据：Codex 5H 与 7D 窗口的剩余额度比例、各自的本地重置时间与距重置时刻的
  剩余时长，以及必要的状态提示。
- 交互方式：BOOT 按键与 GPIO13 高电平外接按键均切换服务商额度页面。
- 首版服务商：Codex。
...
多个额度窗口各自保留窗口标签和重置时间，不合并成一个数字。重置行在绝对时间后
附加一位小数倒计时：`5H` 窗口按小时（如 `(3.7h)`），`7D`／`1W` 窗口按天（如
`(4.6d)`）；`reset_in_sec` 缺失或窗口标签未映射时只显示绝对时间。
...
esp32 固件                 按当前页面轮询，分服务商保存独立快照
    ↓
SH1106 128x64 OLED         BOOT／GPIO13 按键切换 Codex／智谱双窗口页面
```
...
（内部上拉，按下为低电平）；外接高电平按键的 OUT 接 GPIO13（内部下拉，按下为
高电平）。两颗按键各自执行 20 ms 去抖，任一按键稳定按下都会切换到下一服务商页面，
长按不会重复触发。两颗按键的单击双向切页和长按单次触发均已通过实机验收。
...
状态、错误响应、恢复、中文布局和重置倒计时的实机验证。后续新增的 GPIO13 外接按键
已通过软件语法检查、Arduino IDE 编译、烧录、单击双向切页和长按单次触发验证。
