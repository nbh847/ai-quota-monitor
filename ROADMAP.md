...
v0.1 已完成。PC Agent 通过语法检查、41 项单元测试、真实 Codex 额度查询和 HTTP 冒烟；
ESP32-S3 固件通过只读 `-fsyntax-only` 自检、Arduino IDE 编译与烧录，并完成双窗口页面、
进度条、重置时间、同步／过期状态、BOOT、停止 Agent、断网、错误响应及恢复的实机验收。
本地 `secrets.h` 保持 Git 忽略，提交集不含真实配置或账号凭据。v0.1 的 HTTP 接口冻结为
仅可信局域网使用，不提供应用层鉴权，不应暴露到公网或不受信任网络。
